...
/**
 */
public class LuceneRealtimePlumber implements Plumber
{
  private static final EmittingLogger log = new EmittingLogger(LuceneRealtimePlumber.class);
  ...
  private final Map<Long, LuceneSink> sinks = Maps.newConcurrentMap();
  private final VersionedIntervalTimeline<String, LuceneSink> sinkTimeline = new VersionedIntervalTimeline<String, LuceneSink>(
  ...
  public Map<Long, LuceneSink> getSinks()
  {
    return sinks;
  }
  ...
  {
    final LuceneSink sink = getSink(row.getTimestampFromEpoch());
    if (sink == null) {
      return -1;
    }
  ...
  private LuceneSink getSink(long timestamp)
  {
    ...
    LuceneSink retVal = sinks.get(truncatedTime);
    ...
    if (retVal == null) {
      ...
      retVal = new LuceneSink(sinkInterval, schema, config, versioningPolicy.getVersion(sinkInterval));
      addSink(retVal);
  ...
  public <T> QueryRunner<T> getQueryRunner(final Query<T> query)
  {
    ...
    List<TimelineObjectHolder<String, LuceneSink>> querySinks = Lists.newArrayList();
    ...
    return toolchest.mergeResults(
        factory.mergeRunners(
            ...
            FunctionalIterable
                .create(querySinks)
                .transform(
                    new Function<TimelineObjectHolder<String, LuceneSink>, QueryRunner<T>>()
                    {
                      @Override
                      public QueryRunner<T> apply(TimelineObjectHolder<String, LuceneSink> holder)
                      {
                        if (holder == null) {
                        ...
                        final LuceneSink theSink = holder.getObject().getChunk(0).getObject();
                        final boolean skipIncrementalSegment = query.getContextValue(SKIP_INCREMENTAL_SEGMENT, false);
  ...
  public void persist(final Committer committer)
  {
    ...
    for (LuceneSink sink : sinks.values()) {
      if (sink.swappable()) {
  ...
  // Submits persist-n-merge task for a LuceneSink to the mergeExecutor
  private void persistAndMerge(final long truncatedTime, final LuceneSink sink)
  {
        ...
        new ThreadRenamingRunnable(threadName)
        {
          ...
          public void doRun()
          {
            ...
            if (sinks.get(truncatedTime) != sink) {
              log.info("LuceneSink[%s] was abandoned, bailing out of persist-n-merge.", sink);
              return;
            }
            ...
            for (FireHydrant hydrant : sink) {
              synchronized (hydrant) {
                if (!hydrant.hasSwapped()) {
                  log.info("Hydrant[%s] hasn't swapped yet, swapping. LuceneSink[%s]", hydrant, sink);
  ...
  public void finishJob()
  {
    ...
    for (final Map.Entry<Long, LuceneSink> entry : sinks.entrySet()) {
      ...
      try {
        log.info(
            ...
            Joiner.on(", ").join(
                Iterables.transform(
                    sinks.values(),
                    new Function<LuceneSink, String>()
                    {
                      @Override
                      public String apply(LuceneSink input)
                      {
  ...
  protected Object bootstrapSinksFromDisk()
  {
    ...
    for (File sinkDir : files) {
      ...
      final LuceneSink currSink = new LuceneSink(sinkInterval, schema, config, versioningPolicy.getVersion(sinkInterval), hydrants);
      addSink(currSink);
    }
    return metadata;
  }

  private void addSink(final LuceneSink sink)
  {
    sinks.put(sink.getInterval().getStartMillis(), sink);
    sinkTimeline.add(
        sink.getInterval(),
        sink.getVersion(),
        new SingleElementPartitionChunk<LuceneSink>(sink)
    );
    try {
  ...
  private void mergeAndPush()
  {
    ...
    List<Map.Entry<Long, LuceneSink>> sinksToPush = Lists.newArrayList();
    for (Map.Entry<Long, LuceneSink> entry : sinks.entrySet()) {
    ...
    for (final Map.Entry<Long, LuceneSink> entry : sinksToPush) {
      persistAndMerge(entry.getKey(), entry.getValue());
    }
  }
   ...
   */
  protected void abandonSegment(final long truncatedTime, final LuceneSink sink)
  {
  ...
  private void removeSegment(final LuceneSink sink, final File target)
  {
    if (target.exists()) {
      try {
